...
class AddListNexus(object):
    ...
    def __init__(
        ...
        if len(list_nexus) == 1:
            ...
            LoadNexus(filename=_filename, output_wks=_ws_name, metadata_only=metadata_only)
            self.wks = _ws_name

        else:
            load_list_object = LoadListNexus(
                list_nexus=list_nexus, list_run=list_run, metadata_only=metadata_only, prefix=self.prefix
            )
