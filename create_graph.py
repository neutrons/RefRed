...
for line in dot.splitlines():
    if (
        line.startswith('"quicknxs"')
        or 'version' in line
        or '"logging"' in line
        or 'sip' in line
        or 'figureoptions' in line
        or 'types' in line
        or 'nxutils.so' in line
        or 'output_templates' in line
        or 'os' in line
        or 'sys' in line
    ):
        continue
    elif line.startswith('"quicknxs/'):
        line = line.replace('style=filled', 'style=filled, color="#00aa00" shape=box, fontsize=24').replace(
            'quicknxs/', ''
        )
    if 'PyQt4' in line or 'email' in line or 'scipy' in line or 'matplotlib/' in line or 'IPython' in line:
        line = line.split('/')[0] + '";'
        if first_add:
            first_add = False
...
Popen(['dot', '-Tps', '-o', 'import_graph.ps'], stdout=PIPE, stdin=PIPE).communicate(out_dot)
