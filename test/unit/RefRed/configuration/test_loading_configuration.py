...
class TestLoadingConfiguration(object):
    ...
    def test_run_file_found(self, mockQFileDialog, mockLoading, mockStatusMessageHandler, mockOsPathIsFile):
        ...
        mockStatusMessageHandler.assert_called_with(parent=loadingConfiguration.parent, message='Done!', is_threaded=True)
    ...
    def test_run_file_not_found(self, mockQFileDialog, mockLoading, mockStatusMessageHandler, mockOsPathIsFile):
        ...
        mockStatusMessageHandler.assert_called_with(parent=loadingConfiguration.parent, message='User Canceled loading!', is_threaded=True)
    ...
    @mock.patch('RefRed.configuration.loading_configuration.LoadingConfiguration.loading')
    @mock.patch('qtpy.QtWidgets.QFileDialog')
    def test_run_file_found_but_filename_is_list(self, mockQFileDialog, mockLoading, mockStatusMessageHandler, mockOsPathIsFile):
        mockFileDialog = mock.Mock()
        mockFileDialog.exec_.return_value = True
        mockFileDialog.selectedFiles.return_value = ['', 'this is a filename']
    ...
    def test_getMetadataObject(self):
        ...
        values = {'from_peak_pixels': 1.001,
                  ...
                  'clocking_from' : 7.007,
                  'clocking_to' : 8.008,
                  'from_tof_range' : 9.009,
                  'to_tof_range' : 10.010,
                  'from_q_range' : 11.011,
                  'to_q_range' : 12.012,
                  'from_lambda_range' : 13.013,
                  'to_lambda_range' : 14.014,
                  'data_sets' : 'dataset1, dataset2',
                  'tof_range_flag' : 'tof_range_flag',
                  'norm_flag' : 'norm_flag',
                  'norm_from_peak_pixels' : 15.015,
                  'norm_to_peak_pixels' : 16.016,
                  'norm_from_back_pixels' : 17.017,
                  'norm_to_back_pixels' : 18.018,
